# release-baseline-numbers Specification

## Purpose
Govern how the benchmark numbers a release quotes are produced, what provenance
must accompany them, where they are stored so they outlive a scratch directory,
and the rule that every reader-facing performance claim is traceable to a
published run that was not edited after the fact.
