...
Register external catalogs (Hive Metastore, or Iceberg tables tracked by a Hive Metastore):
...
| `type` | string | yes | `"hive"` (Hive tables) or `"iceberg"` (Iceberg tables in the same kind of metastore; see [Iceberg](/connectors/iceberg)) |
