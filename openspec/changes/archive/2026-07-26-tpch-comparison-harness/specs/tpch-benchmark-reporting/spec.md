...
This change SHALL remove `benchmarks/tpch/scripts/report.py`, replaced by the `tpch-bench report` subcommand.

The shell script `benchmarks/tpch/scripts/run_benchmark.sh` MUST be updated to invoke `tpch-bench report` instead of `python3 scripts/report.py`.
