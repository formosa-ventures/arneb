# tpch-benchmark-tutorial Specification

## Purpose
Give a newcomer a single document that carries them from a clean checkout to an
interpreted three-way comparison report, so the claim "a Trino alternative built in
Rust" is something a reader can verify on their own machine instead of taking on
faith. The tutorial must stay in lockstep with what the runner actually emits —
drift between the documented report schema and the real one is the failure mode
